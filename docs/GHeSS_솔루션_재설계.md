...
작성 기준: 2026-09-18 · 상태: 임베딩 전량 완료 이후의 설계 제안 · 원문: 루트의 「GHeSS AI Agent 서비스 아키텍처 초안」, 작성 기준 2026-08-03, 24쪽.

**진행 갱신:** 사용자 확인으로 2026-09-15 임베딩 대상 약 61,000여 문서, 총 7,585,436개 청크의 임베딩이 완료됐다. 남은 우선 작업은 프로덕션 Milvus 이관, 메타데이터 보강, 샤드·인덱스/검색 성능 검증이다. 구체적인 순서와 전환·복귀 조건은 [Milvus 프로덕션 마이그레이션 및 검색 최적화 계획서](Milvus_프로덕션_이관_및_검색_최적화.md)를 따른다.
...
| 사용자 확인 · 2026-09-15 | 임베딩 대상 약 61,000여 문서·7,585,436개 청크 완료; 프로덕션 컬렉션 이관과 메타데이터 확장 예정 | 완료 배치 manifest와 PK 집합을 대사하고 기존 벡터를 재사용해 이관·검증 |
| 사용자 확인 | 임베딩 모델 BGE-M3, 현재 유사도 척도 COSINE | 모델과 COSINE을 유지해 이관; 표준 dense 출력 1,024차원을 실제 schema와 대조 |
...
| 미확인 | 7,585,436개 중 유효 PK·이력·삭제 범위, 저장 차원/dtype·전처리·dense 외 출력 보존 여부, Milvus 버전·인덱스·복제·자원 | BGE-M3 표준 dense 1,024차원으로 계획하고 실제 schema·대사 결과·RAM으로 보정 |
...
BGE-M3의 표준 dense 출력 1,024차원은 [공식 모델 카드](https://huggingface.co/BAAI/bge-m3)에서 확인했다. 사용자 환경의 실제 schema를 읽은 결과는 아니다. 이후 표의 숫자·구성은 명시적으로 확정이라고 표시한 항목 외에는 **제안 또는 계획 가정**이다. 다른 경로에서 확보한 실제 구현 자료를 검토한 것은 아니다.
...
임베딩 전량 완료 이후의 실제 이관 경로, 필드 설계, 샤드·파티션·인덱스 후보, 야간 분할 일정, OOM 통제와 인프라 문의 사항은 [후속 계획서](Milvus_프로덕션_이관_및_검색_최적화.md)에 구체화했다. 원본 서버의 별도 컬렉션에서 메타데이터 개편과 대사를 끝낸 뒤 컬렉션 단위로 프로덕션에 복원하는 안을 우선 검토한다.
...
확인된 BGE-M3 dense 벡터의 COSINE 검색을 기준으로 HNSW 후보 `M=16`, `efConstruction=200`, 검색 `ef=64/128/256` 등을 비교한다. 이는 권장 확정값이 아니다. 현재 인덱스가 이미 SLA를 만족하면 그대로 둔다. 필터 선택도 1%/10%/100%, 일반 질의/문서번호, 동시 검색 및 삽입 중 p95·recall을 비교한다. [HNSW](https://milvus.io/docs/hnsw.md).
...
- 확인된 7,585,436개 청크에 BGE-M3 표준 dense 1,024차원·float32 저장을 가정하면 원시 벡터는 약 28.94GiB다. 현재 산정 계수의 질의 복제본당 RAM 계획치는 약 101GiB이므로 128GiB 이상을 보수적 검증 시작점으로 두고 실제 schema·HNSW·load peak로 보정한다.
- 95GB 원본과 7,585,436개 청크를 기준으로 추출 이미지·개정·검색 인덱스·백업·재색인 공간을 분리하면 기준 시나리오의 전체 논리 할당은 약 1,336.6GiB다. 새로 전부 할당하라는 뜻이 아니며 기존 G-HeSS·스토리지의 보유 자원을 차감해야 한다.
...
|---|---|---|
| A. 완료 상태 계측 | 2026-09-15 완료 manifest, 포팅 버전, schema/index, 7,585,436개 청크와 PK, OCR 샘플, 사내 모델/API 한도 수집 | 정확한 대상/PK·환경 명세·기준 성능/품질 보고서 |
| B. 메타 보강·프로덕션 이관 | 문서 ID·개정·출처·OCR quality 보강, 기존 벡터와 결합해 대상 컬렉션 적재, 샤드·인덱스 비교 | 건수/PK/벡터 대사, 원문 위치·개정 연결, index/load·품질·부하·복귀 검증 |
...
| 변경 | 파싱/OCR | 임베딩 | 적용 방법 |
|---|---|---|---|
| 프로덕션 서버 이관 | 없음 | 기존 벡터 재사용 | 호환성 검증한 복원 또는 export/import, 증분 반영·전환·복귀 시험 |
| 동일 로직 전체 재임베딩 | 보존된 청크 텍스트 사용 | 대상 전량 | 기존 생성 계약·무결성이 불확실할 때 pilot 비교; 현재 계획 가정 약 75.3시간 |
